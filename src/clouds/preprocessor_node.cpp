...
static std::string nodeNS = "/preprocessor/kinect1";   //"/preprocessor";
static std::string nodeName = "/preprocessor_node";
string LocalConfig::inputTopic = "/kinect1/depth_registered/points";  //"/camera/rgb/points";
...
string LocalConfig::backgroundFile = std::string(getenv("BULLETSIM_SOURCE_DIR")) + "/data/preprocessor.yml";
...
public:
	...
	void callback(const sensor_msgs::PointCloud2& msg_in) {
		...
		//FIXME Hard-coded threshold  not using LocalConfig::removeOutliers parameters
	...
	void initTable(ColorCloudPtr cloud) {
		MatrixXf corners = getTableCornersRansac(cloud);    // 4*3 corner points
