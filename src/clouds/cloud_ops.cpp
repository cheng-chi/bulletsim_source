...
ColorCloudPtr smoothSurface(const ColorCloudPtr in) {
	...
	pcl::MovingLeastSquares<ColorPoint, ColorPoint> mls;
...
cv::Mat aboveTableMask(const ColorCloudPtr in, cv::Mat mask, pcl::ModelCoefficients::Ptr coefficients, float threshold) {
	// threshold: above table --> positive

	assert(mask.rows == in->height);
	assert(mask.cols == in->width);

	vector<float> coeffs = coefficients->values;
	cv::Mat mask_out = mask;

	bool coeff_z_pos = coeffs[2]>0;

	if (coeff_z_pos){
		for (int i=0; i<in->height; i++) {
				for (int j=0; j<in->width; j++) {
					if (mask_out.at<bool>(i,j)) {
						if (  in->at(in->width*i+j).x * coeffs[0]
							+ in->at(in->width*i+j).y * coeffs[1]
							+ in->at(in->width*i+j).z * coeffs[2]
							+ 1.0 * coeffs[3]
									 > -1*threshold ){
							mask_out.at<bool>(i,j) = false;
						}
					}
				}
			}
...
void labelCloud(ColorCloudPtr in, const cv::Mat& labels) {
	...
	for (int i=0; i < in->size(); i++)
		...
		in->points[i].a = labels.at<uint8_t>(uv(i,0), uv(i,1));
}
