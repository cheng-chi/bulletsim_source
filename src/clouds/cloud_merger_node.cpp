...
void broadcastTransformCallback(const sensor_msgs::PointCloud2ConstPtr& msg_in, int i) {
	try {
	  if (i == LocalConfig::tfParent) {
        ...
        broadcaster->sendTransform(tf::StampedTransform(toTFTransform(toBulletTransform(tfTransform) * toBulletTransform((Affine3f)transforms[i]).inverse()), ros::Time::now(), link,  "chess_board"));
	  }
	  else {
