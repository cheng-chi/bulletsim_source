...
public:
  ...
  CapsuleRope(const std::vector<btVector3>& ctrlPoints, float radius_, float angStiffness_=.1, float angDamping_=1, float linDamping_=.75, float angLimit_=.8, float linStopErp_=.2);
  void init();
  void destroy();
